...
// Public Functions...
//KNOEDELstart
/* pyrrh: mostly copied from unmodded resolveCombat; called in updateCombat;
changes (see also inline):
identify unit with odds better than PYRRH_THRESH-NO-KILL (GlobalDefines.xml) upfront,
    mark it as unkillable
that unit retreats rather than being defeated (as if having 100% withdrawal chance)
if it's the defender who is defeated against the odds, the attacker retreats instead
normal withdrawals boosted in CvInfos, no change here
XP changes implemented here
war weariness handled in "aftermath" section of the updateCombat function */
int CvUnit::resolveCombat_pyrrh(CvUnit* pDefender, CvPlot* pPlot, CvBattleDefinition& kBattle)
{
    ...
    // <pyrrh>
    bool attUnkillable, defUnkillable;
    attUnkillable = defUnkillable = false;
    int oddsAtt_PerMill = getCombatOdds(this, pDefender);
    int oddsDef_PerMill = 1000 - oddsAtt_PerMill;
        /*    oddsDef_PerMill *= ((100 - withdrawalProbability()) / 100)); <-- could do this
              (and round the result). current stance:
              withdrawal chance should not matter when deciding whether
              defender can get killed. only chance of defender (not) being killed should matter.*/
    int thresh_PerMill = 10 * GC.getDefineINT("PYRRH_THRESH-NO-KILL");
    if(oddsAtt_PerMill > thresh_PerMill)
        attUnkillable = true;
    if(oddsDef_PerMill > thresh_PerMill)
        defUnkillable = true;
    int r = 0; // return value indicating retreat and who got unlucky
    // </pyrrh>
    ...
    while (true)
    {
        if (GC.getGameINLINE().getSorenRandNum(GC.getDefineINT("COMBAT_DIE_SIDES"), "Combat") < iDefenderOdds)
        {
            if (getCombatFirstStrikes() == 0)
            {
                ...
                {
                   if(GC.getGameINLINE().getSorenRandNum(100, "Withdrawal")
                        < withdrawalProbability()) {
                      // pyrrh: withdrawal handled as before:
                   ...
                   // <pyrrh>: retreat
                   else if(attUnkillable) {
                    /* moved from below the isDead check at the end of the function
                       defender gains XP as for a kill; attacker a small amount */
                       r = 1;
                    int iExperience = defenseXPValue();
                    /* this could be done in XML (CIV4UnitInfos.xml), but would require changing dozens
                       of elements. */
                    if(iExperience == 2) 
...
void CvUnit::reset(int iID, UnitTypes eUnit, PlayerTypes eOwner, bool bConstructorCall)
{
    ...
    retreat = 0;	//KNOEDEL	// pyrrh: probably unnecessary (unless games are somehow saved in the middle of combat)
}
...
//KNOEDELstart <pyrrh> check if mod is enabled, else call unmodded resolveCombat
            retreat = 0;
            if(GC.getDefineINT("PYRRH_THRESH-NO-KILL") >= 50)
                retreat = resolveCombat_pyrrh(pDefender, pPlot, kBattle);
            else // </pyrrh>
//KNOEDELend
	...
	if (bFinish)
	{
		...
		{
//KNOEDELstart
            /* <pyrrh>: aftermath of retreats; call the unit that survived
               against the odds "tactical winner" */
            bool isAttackerTacticalWinner = true;
            TeamTypes eTacticalWinner = getTeam();
            TeamTypes eOther = pDefender->getTeam();
            if(retreat == 1) { // 1 means the attacker was superior but unlucky
                isAttackerTacticalWinner = false;
                eTacticalWinner = pDefender->getTeam();
                eOther = getTeam();
            }
            CvTeam& tacticalWinner = GET_TEAM(eTacticalWinner);
            CvTeam& other = GET_TEAM(eOther);
            bool hiddenNationality = m_pUnitInfo->isHiddenNationality()
                    || pDefender->getUnitInfo().isHiddenNationality();
            if(retreat != 0 && !hiddenNationality) {
                // war weariness; four cases: who attacked? who is the (tactical) winner?
                int const wwBeatAttacker = GC.getDefineINT("WW_KILLED_UNIT_DEFENDING");
                int const wwBeatDefender = GC.getDefineINT("WW_KILLED_UNIT_ATTACKING");
                int const wwLostToDefender = GC.getDefineINT("WW_UNIT_KILLED_ATTACKING");
                int const wwLostToAttacker = GC.getDefineINT("WW_UNIT_KILLED_DEFENDING");
                /* tell the non-winner to adjust its ww against the winner due to a tactical defeat,
                   and tell the winner to adjust its ww against the non-winner due to a tactical victory. */
                other.changeWarWeariness(eTacticalWinner, *pPlot,
                        isAttackerTacticalWinner ? wwLostToAttacker : wwLostToDefender);
                tacticalWinner.changeWarWeariness(eOther, *pPlot,
                        isAttackerTacticalWinner ? wwBeatDefender : wwBeatAttacker);
            }
            // tell player that his attacker withdrew or -- new -- retreated.
            CvWString msgTag = (retreat != 0 ? "TXT_KEY_PYRRH_YOU_UNIT_RETREAT"
                    : "TXT_KEY_MISC_YOU_UNIT_WITHDRAW");
            // </pyrrh>
            szBuffer = gDLL->getText(msgTag, getNameKey(), pDefender->getNameKey());
            gDLL->getInterfaceIFace()->addMessage(getOwnerINLINE(), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_OUR_WITHDRAWL", MESSAGE_TYPE_INFO, NULL, (ColorTypes)GC.getInfoTypeForString(
    isAttackerTacticalWinner ? "COLOR_GREEN" : "COLOR_RED" // pyrrh: instead of COLOR_GREEN
                ), pPlot->getX_INLINE(), pPlot->getY_INLINE());
            // <pyrrh>: tell other player that the attacker withdrew or retreated.
            msgTag = (retreat != 0 ? "TXT_KEY_PYRRH_ENEMY_UNIT_RETREAT"
                    : "TXT_KEY_MISC_ENEMY_UNIT_WITHDRAW");
            // </pyrrh>

            szBuffer = gDLL->getText(msgTag, getNameKey(), pDefender->getNameKey());
            gDLL->getInterfaceIFace()->addMessage(pDefender->getOwnerINLINE(), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_THEIR_WITHDRAWL", MESSAGE_TYPE_INFO, NULL, (ColorTypes)GC.getInfoTypeForString(
    isAttackerTacticalWinner ? "COLOR_RED" : "COLOR_GREEN" // pyrrh: instead of COLOR_RED
                ), pPlot->getX_INLINE(), pPlot->getY_INLINE());
//KNOEDELend
...
//KNOEDELstart
	...
	if (pPlot->getImprovementType() != NO_IMPROVEMENT)
	{
		...
		if (iPillageGold > 0)
		{
			...
			szBuffer = gDLL->getText("TXT_KEY_MISC_GOLD_FROM_SETTLE_IMP", iPillageGold, GC.getImprovementInfo(pPlot->getImprovementType()).getTextKeyWide());
			gDLL->getInterfaceIFace()->addMessage(getOwnerINLINE(), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_PILLAGE", MESSAGE_TYPE_INFO, getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_GREEN"), pPlot->getX_INLINE(), pPlot->getY_INLINE());
