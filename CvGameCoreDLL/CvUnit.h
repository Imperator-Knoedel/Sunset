...
//KNOEDELstart
    ...
    /* modded combat resolution. @return: 0 when no unit retreated; otherwise retreat:
	   1 if a superior attacker was unlucky and only saved by the mod,
	  -1: same for the defender. */
    int resolveCombat_pyrrh(CvUnit* pDefender, CvPlot* pPlot, CvBattleDefinition& kBattle);
	private: int retreat; /* has to be a data member in order to remember retreat results
						     in between calls to 'updateCombat'. the function is entered
							 repeatedly while 3D animations are shown. */
	public: // </pyrrh>
//KNOEDELend
