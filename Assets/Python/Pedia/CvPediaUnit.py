...
class CvPediaUnit:
	...
	def placeInfo(self):
		...
		if gc.getUnitInfo(self.iUnit).getAirCombat() > 0 and gc.getUnitInfo(self.iUnit).getCombat() > 0:
			szStats += u"  %d%c" % (gc.getUnitInfo(self.iUnit).getAirCombat(), CyGame().getSymbolID(FontSymbols.BULLET_CHAR))		

		# Movement
		if UnitInfo.getAirRange() > 3:
			szStats += u"  %d%c" % (UnitInfo.getAirRange(), CyGame().getSymbolID(FontSymbols.BULLET_CHAR))
		else:
			szStats += u"  %d%c" % (UnitInfo.getMoves(), CyGame().getSymbolID(FontSymbols.MOVES_CHAR))
		
		# Range
		if UnitInfo.getAirRange() > 0 and UnitInfo.getAirRange() < 4:
			szStats += u"  %d%c" % (UnitInfo.getAirRange(), CyGame().getSymbolID(FontSymbols.BULLET_CHAR))
